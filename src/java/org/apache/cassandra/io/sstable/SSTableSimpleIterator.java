...
public abstract class SSTableSimpleIterator extends AbstractIterator<Unfiltered> implements Iterator<Unfiltered>
{
    ...
    private static class CurrentFormatIterator extends SSTableSimpleIterator
    {
        ...
        private CurrentFormatIterator(CFMetaData metadata, DataInput in, SerializationHeader header, SerializationHelper helper)
        {
            ...
            this.row = new ReusableRow(clusteringSize, regularColumns, true, helper.nowInSec, metadata.isCounter());
        ...
        protected Unfiltered computeNext()
        {
            try
            {
                Unfiltered.Kind kind = UnfilteredSerializer.serializer.deserialize(in, header, helper, row.writer(), marker.writer());

                return kind == null
                     ? endOfData()
                     : (kind == Unfiltered.Kind.ROW ? row : marker);
            }
            catch (IOException e)
            {
                throw new IOError(e);
