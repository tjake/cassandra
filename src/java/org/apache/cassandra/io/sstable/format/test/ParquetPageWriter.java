...
public class ParquetPageWriter implements PageWriteStore
{
    ...
    public static final LZ4Factory lz4Factory = LZ4Factory.fastestInstance();
    static final LZ4Compressor compressor = lz4Factory.fastCompressor();
    public static final LZ4FastDecompressor decompressor = lz4Factory.fastDecompressor();
    ...
    public void write(SequentialWriter out, long numRows) throws IOException
    {
        ...
        {
            //Don't write null pages
            if (entry.getValue().getPages().isEmpty())
                continue;
            ...
            //Write each page, we avoid writing empty columns chunks
            for (Page p : entry.getValue().getPages())
            {
                ...
                int bytesSize = p.getUncompressedSize();
                int compressedSize = p.getUncompressedSize();
                byte[] bytes = p.getBytes().toByteArray();

                //Write the header
                PageHeader pageHeader = new PageHeader(PageType.DATA_PAGE, bytesSize, compressedSize);
                ...
                totalUncompressedSize += bytesSize;


                byte[] ph = null;
                try
                {
                    ph = ser.serialize(pageHeader);
                    out.write(ph);
                } catch (TException e)
                {
                    throw new IOException(e);
            ...
            if (rowGroup != null)
            {
                ...
                ColumnMetaData cm = new ColumnMetaData(getType(desc.getType()), Lists.newArrayList(encodings), Arrays.asList(desc.getPath()),
                        CompressionCodec.LZO, numValues, totalUncompressedSize, totalUncompressedSize, dataPageOffset);

                //Write the column metadata
                byte[] cmd = null;
                try
                {
                    cmd = ser.serialize(cm);
                    out.write(cmd);
                } catch (TException e)
                {
                    throw new IOException(e);
