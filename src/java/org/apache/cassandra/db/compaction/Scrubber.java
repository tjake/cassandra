        ...
        try
        {
            ...
            while (!dataFile.isEOF())
            {
                ...
                try
                {
                    ...
                    try (AtomIterator atoms = AtomIterators.withValidation(new SSTableIdentityIterator(sstable, dataFile, key, nowInSec), dataFile.getPath()))
                    {
                ...
                catch (Throwable th)
                {
                        ...
                        && (key == null || !key.getKey().equals(currentIndexKey) || dataStart != dataStartFromIndex || dataSize != dataSizeFromIndex))
                    {
                        ...
                        try
                        {
                            AtomIterator atoms = AtomIterators.withValidation(new SSTableIdentityIterator(sstable, dataFile, key, nowInSec), dataFile.getPath());
                            if (prevKey != null && prevKey.compareTo(key) > 0)
                            {
                                saveOutOfOrderRow(prevKey, key, atoms);
                                continue;
