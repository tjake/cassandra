...
public class HintedHandOffManager implements HintedHandOffManagerMBean
{
    ...
    private void doDeliverHintsToEndpoint(InetAddress endpoint)
    {
        ...
        try (RowIterator iter = UnfilteredRowIterators.filter(SinglePartitionReadCommand.fullPartitionRead(SystemKeyspace.Hints, nowInSec, epkey).queryMemtableAndDisk(hintStore)))
        {
