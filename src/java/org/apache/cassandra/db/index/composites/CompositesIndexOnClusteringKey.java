...
public class CompositesIndexOnClusteringKey extends CompositesIndex
{
    ...
    {
        return column.name().get(columnDef.componentIndex);
    }

    protected Composite makeIndexColumnPrefix(ByteBuffer rowKey, Composite columnName)
    {
        int count = Math.min(baseCfs.metadata.clusteringKeyColumns().size(), columnName.size());
        CBuilder builder = getIndexComparator().builder();
        builder.add(rowKey);
        for (int i = 0; i < Math.min(columnDef.componentIndex, count); i++)
            builder.add(columnName.get(i));
        for (int i = columnDef.componentIndex + 1; i < count; i++)
            builder.add(columnName.get(i));
