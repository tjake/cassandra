...
public abstract class LegacyLayout
{
    ...
    public static Row extractStaticColumns(CFMetaData metadata, DataInput in, Columns statics, int nowInSec) throws IOException
    {
        ...
        StaticRow.Builder builder = StaticRow.builder(statics, false, nowInSec, metadata.isCounter());
    ...
    public static class CellGrouper
    {
        ...
        private CellGrouper(CFMetaData metadata, SerializationHelper helper, boolean isStatic)
        {
            ...
            this.helper = helper;
            this.row = isStatic ? null : new ReusableRow(metadata.clusteringColumns().size(), metadata.partitionColumns().regulars, false, helper.nowInSec, metadata.isCounter());

            if (isStatic)
                this.writer = StaticRow.builder(metadata.partitionColumns().statics, false, helper.nowInSec, metadata.isCounter());
        }
