...
public abstract class UnfilteredDeserializer
{
    ...
    private static class CurrentDeserializer extends UnfilteredDeserializer
    {
                                    ...
                                    SerializationHelper helper)
        {
            ...
            this.row = new ReusableRow(metadata.clusteringColumns().size(), header.columns().regulars, true, helper.nowInSec, metadata.isCounter());
        ...
        public Unfiltered readNext() throws IOException
        {
            ...
            if (UnfilteredSerializer.kind(nextFlags) == Unfiltered.Kind.RANGE_TOMBSTONE_MARKER)
            {
                ...
                UnfilteredSerializer.serializer.deserializeMarkerBody(in, header, helper, nextFlags, writer);
                return marker;
            }
            else
            {
        ...
        public void skipNext() throws IOException
        {
            ...
            if (UnfilteredSerializer.kind(nextFlags) == Unfiltered.Kind.RANGE_TOMBSTONE_MARKER)
            {
                UnfilteredSerializer.serializer.skipMarkerBody(in, header, nextFlags);
            }
            else
            {
