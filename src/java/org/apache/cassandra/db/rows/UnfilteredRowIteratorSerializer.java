...
public class UnfilteredRowIteratorSerializer
{
    ...
    public Header deserializeHeader(DataInput in, int version, SerializationHelper.Flag flag) throws IOException
    {
        ...
        if ((flags & IS_EMPTY) != 0)
        {
            SerializationHeader sh = new SerializationHeader(metadata, PartitionColumns.NONE, RowStats.NO_STATS, false);
        ...
        if (hasStatic)
            staticRow = UnfilteredSerializer.serializer.deserializeStaticRow(in, header, new SerializationHelper(version, flag, nowInSec));
    ...
    public UnfilteredRowIterator deserialize(final DataInput in, int version, SerializationHelper.Flag flag) throws IOException
    {
        ...
        if (h.isEmpty)
            return UnfilteredRowIterators.emptyIterator(h.metadata, h.key, h.isReversed, h.nowInSec);
        ...
        return new AbstractUnfilteredRowIterator(h.metadata, h.key, h.partitionDeletion, h.sHeader.columns(), h.staticRow, h.isReversed, h.sHeader.stats(), h.nowInSec)
        {
            private final ReusableRow row = new ReusableRow(clusteringSize, h.sHeader.columns().regulars, true, helper.nowInSec, h.metadata.isCounter());
            ...
            protected Unfiltered computeNext()
            {
                try
                {
                    Unfiltered.Kind kind = UnfilteredSerializer.serializer.deserialize(in, h.sHeader, helper, row.writer(), marker.writer());
                    if (kind == null)
                        return endOfData();

                    return kind == Unfiltered.Kind.ROW ? row : marker;
                }
                catch (IOException e)
                {
                    throw new IOError(e);
    ...
    public static class Header
    {
        ...
        {
            return String.format("{header=%s, table=%s.%s, key=%s, isReversed=%b, isEmpty=%b, del=%s, staticRow=%s, rowEstimate=%d, nowInSec=%d}",
                                 sHeader, metadata.ksName, metadata.cfName, key, isReversed, isEmpty, partitionDeletion, staticRow.toString(metadata), rowEstimate, nowInSec);
        }
    }
}
