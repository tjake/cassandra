...
public abstract class WrappingRow extends AbstractRow
{
    ...
    public Row takeAlias()
    {
        ...
        if (isStatic())
        {
            StaticRow.Builder builder = StaticRow.builder(columns(), true, nowInSec(), isCounter);
            copyTo(builder);
            return builder.build();
        }
        else
        {
            ReusableRow copy = new ReusableRow(clustering().size(), columns(), true, nowInSec(), isCounter);
            copyTo(copy.writer());
            return copy;
        }
    }
