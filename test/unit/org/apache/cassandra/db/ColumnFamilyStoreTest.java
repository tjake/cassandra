...
public class ColumnFamilyStoreTest extends SchemaLoader
{
    ...
    public void testEmptyRow() throws Exception
    {
        ...
        Runnable r = new WrappedRunnable()
        {
            ...
            {
                QueryFilter sliceFilter = QueryFilter.getSliceFilter(Util.dk("key1"), "Standard2", Composites.EMPTY, Composites.EMPTY, false, 1);
    ...
    public void testIndexScan() throws IOException
    {
        ...
        rows = cfs.search(clause, range, 100, Util.namesFilter(cfs, "birthdate"));
        ...
        rows = cfs.search(clause, range, 100, emptyFilter);
    ...
    public void testDeleteOfInconsistentValuesInKeysIndex() throws Exception
    {
        ...
        rm.apply();
        IndexExpression expr = new IndexExpression(ByteBufferUtil.bytes("birthdate"), IndexOperator.EQ, val1);
        List<IndexExpression> clause = Arrays.asList(expr);
        IDiskAtomFilter filter = new IdentityQueryFilter();
        Range<RowPosition> range = Util.range("", "");
        ...
        expr = new IndexExpression(ByteBufferUtil.bytes("birthdate"), IndexOperator.EQ, val2);
        clause = Arrays.asList(expr);
    ...
    public void testDeleteOfInconsistentValuesFromCompositeIndex() throws Exception
    {
        ...
        CellName compositeName = baseComparator.make(clusterKey, colName);
    ...
    public void testIndexScanWithLimitOne() throws IOException
    {
        ...
        IPartitioner p = StorageService.getPartitioner();
        Range<RowPosition> range = Util.range("", "");
        List<Row> rows = cfs.search(clause, range, 1, filter);
    ...
    {
        ColumnFamilyStore cfs = Table.open("Keyspace2").getColumnFamilyStore("Standard1");
        List<IMutation> rms = new LinkedList<IMutation>();
        RowMutation rm;
    ...
    public void testSliceByNamesCommandOldMetatada() throws Throwable
    {
        ...
        SliceByNamesReadCommand cmd = new SliceByNamesReadCommand(tableName, key.key, cfName, new NamesQueryFilter(FBUtilities.singleton(cname, cfs.getComparator())));
