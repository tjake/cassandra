...
public class TableTest extends SchemaLoader
{
    ...
    public void testGetRowNoColumns() throws Throwable
    {
        ...
        Runnable verify = new WrappedRunnable()
        {
            public void runMayThrow() throws Exception
            {
                ...
                cf = cfStore.getColumnFamily(QueryFilter.getNamesFilter(TEST_KEY, new QueryPath("Standard3"), new TreeSet<CellName>()));
                assertColumns(cf);

                cf = cfStore.getColumnFamily(QueryFilter.getSliceFilter(TEST_KEY, new QueryPath("Standard3"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, false, 0));
                assertColumns(cf);

                cf = cfStore.getColumnFamily(QueryFilter.getNamesFilter(TEST_KEY, new QueryPath("Standard3"), CellName.wrap("col99")));
                assertColumns(cf);
            }
        };
    ...
    public void testGetRowSingleColumn() throws Throwable
    {
        ...
        Runnable verify = new WrappedRunnable()
        {
            public void runMayThrow() throws Exception
            {
                ...
                cf = cfStore.getColumnFamily(QueryFilter.getNamesFilter(TEST_KEY, new QueryPath("Standard1"), CellName.wrap("col1")));
                assertColumns(cf, "col1");

                cf = cfStore.getColumnFamily(QueryFilter.getNamesFilter(TEST_KEY, new QueryPath("Standard1"), CellName.wrap("col3")));
    ...
    public void testGetRowSliceByRange() throws Throwable
    {
        ...
        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("b"), CellName.wrap("c"), false, 100);
        assertEquals(2, cf.getColumnCount());

        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("b"), CellName.wrap("b"), false, 100);
        assertEquals(1, cf.getColumnCount());

        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("b"), CellName.wrap("c"), false, 1);
        assertEquals(1, cf.getColumnCount());
    }
    ...
    public void testGetSliceWithCutoff() throws Throwable
    {
        ...
        Runnable verify = new WrappedRunnable()
        {
            public void runMayThrow() throws Exception
            {
                ...
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col096"), CellName.wrap("col099"), false, 4);
                assertColumns(cf, "col096", "col097", "col098", "col099");

                // test reversed, spanning a segment.
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col099"), CellName.wrap("col096"), true, 4);
                assertColumns(cf, "col096", "col097", "col098", "col099");

                // test forward, within a segment.
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col100"), CellName.wrap("col103"), false, 4);
                assertColumns(cf, "col100", "col101", "col102", "col103");

                // test reversed, within a segment.
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col103"), CellName.wrap("col100"), true, 4);
                assertColumns(cf, "col100", "col101", "col102", "col103");
                ...
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.EMPTY_CELL_NAME, CellName.wrap("col099"), false, 100);
                assertColumns(cf, strCols);

                // test reversed, from end, spanning a segment.
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.EMPTY_CELL_NAME, CellName.wrap("col288"), true, 12);
    ...
    public void testReversedWithFlushing() throws IOException, ExecutionException, InterruptedException
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            cf.addColumn(new Column(CellName.wrap((long)i), ByteBufferUtil.EMPTY_BYTE_BUFFER, 0));
            rm.add(cf);
            rm.apply();
        }
        ...
        for (int i = 10; i < 20; i++)
        {
            ...
            cf = cfs.getColumnFamily(ROW, new QueryPath("StandardLong1"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, true, 1);
            assertEquals(1, cf.getColumnNames().size());
            assertEquals(i, cf.getColumnNames().iterator().next().bb.getLong());
        }
    }
    ...
    private void validateGetSliceNoMatch(Table table) throws IOException
    {
        ...
        cf = cfStore.getColumnFamily(Util.dk("a"), new QueryPath("Standard2"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, false, 1);
        assertColumns(cf);

        // key after the rows that exist
        cf = cfStore.getColumnFamily(Util.dk("z"), new QueryPath("Standard2"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, false, 1);
        assertColumns(cf);
    }
    ...
    public void testGetSliceFromBasic() throws Throwable
    {
        ...
        Runnable verify = new WrappedRunnable()
        {
            public void runMayThrow() throws Exception
            {
                ...
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col5"), CellName.EMPTY_CELL_NAME, false, 2);
                assertColumns(cf, "col5", "col7");

                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col4"), CellName.EMPTY_CELL_NAME, false, 2);
                assertColumns(cf, "col4", "col5", "col7");
                assertColumns(ColumnFamilyStore.removeDeleted(cf, Integer.MAX_VALUE), "col5", "col7");

                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col5"), CellName.EMPTY_CELL_NAME, true, 2);
                assertColumns(cf, "col3", "col4", "col5");

                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col6"), CellName.EMPTY_CELL_NAME, true, 2);
                assertColumns(cf, "col3", "col4", "col5");

                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, true, 2);
                assertColumns(cf, "col7", "col9");

                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col95"), CellName.EMPTY_CELL_NAME, false, 2);
                assertColumns(cf);

                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col0"), CellName.EMPTY_CELL_NAME, true, 2);
                assertColumns(cf);
            }
        };
    ...
    public void testGetSliceWithExpiration() throws Throwable
    {
        ...
        Runnable verify = new WrappedRunnable()
        {
            public void runMayThrow() throws Exception
            {
                ...
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, false, 2);
                assertColumns(cf, "col1", "col2");
                assertColumns(ColumnFamilyStore.removeDeleted(cf, Integer.MAX_VALUE), "col1");

                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col2"), CellName.EMPTY_CELL_NAME, false, 1);
    ...
    public void testGetSliceFromAdvanced() throws Throwable
    {
        ...
        Runnable verify = new WrappedRunnable()
        {
            public void runMayThrow() throws Exception
            {
                ...
                cf = cfStore.getColumnFamily(ROW, new QueryPath("Standard1"), CellName.wrap("col2"), CellName.EMPTY_CELL_NAME, false, 3);
                assertColumns(cf, "col2", "col3", "col4");

                ByteBuffer col = cf.getColumn(CellName.wrap("col2")).value();
                assertEquals(ByteBufferUtil.string(col), "valx");

                col = cf.getColumn(CellName.wrap("col3")).value();
                assertEquals(ByteBufferUtil.string(col), "valx");

                col = cf.getColumn(CellName.wrap("col4")).value();
    ...
    private void validateSliceLarge(ColumnFamilyStore cfStore) throws IOException
    {
        DecoratedKey key = Util.dk("row3");
        ColumnFamily cf;
        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("col1000"), CellName.EMPTY_CELL_NAME, false, 3);
        assertColumns(cf, "col1000", "col1001", "col1002");

        ByteBuffer col;
        col = cf.getColumn(CellName.wrap("col1000")).value();
        assertEquals(ByteBufferUtil.string(col), "v1000");
        col = cf.getColumn(CellName.wrap("col1001")).value();
        assertEquals(ByteBufferUtil.string(col), "v1001");
        col = cf.getColumn(CellName.wrap("col1002")).value();
        assertEquals(ByteBufferUtil.string(col), "v1002");

        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("col1195"), CellName.EMPTY_CELL_NAME, false, 3);
        assertColumns(cf, "col1195", "col1196", "col1197");

        col = cf.getColumn(CellName.wrap("col1195")).value();
        assertEquals(ByteBufferUtil.string(col), "v1195");
        col = cf.getColumn(CellName.wrap("col1196")).value();
        assertEquals(ByteBufferUtil.string(col), "v1196");
        col = cf.getColumn(CellName.wrap("col1197")).value();
        assertEquals(ByteBufferUtil.string(col), "v1197");


        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("col1996"), CellName.EMPTY_CELL_NAME, true, 1000);
        ...
        for (int i = 1000; i < 1996; i++)
        {
            ...
            assertEquals(ByteBufferUtil.string(column.name().bb), expectedName);
            assertEquals(ByteBufferUtil.string(column.value()), ("v" + i));
        }

        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("col1990"), CellName.EMPTY_CELL_NAME, false, 3);
        assertColumns(cf, "col1990", "col1991", "col1992");
        col = cf.getColumn(CellName.wrap("col1990")).value();
        assertEquals(ByteBufferUtil.string(col), "v1990");
        col = cf.getColumn(CellName.wrap("col1991")).value();
        assertEquals(ByteBufferUtil.string(col), "v1991");
        col = cf.getColumn(CellName.wrap("col1992")).value();
        assertEquals(ByteBufferUtil.string(col), "v1992");

        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, true, 3);
        assertColumns(cf, "col1997", "col1998", "col1999");
        col = cf.getColumn(CellName.wrap("col1997")).value();
        assertEquals(ByteBufferUtil.string(col), "v1997");
        col = cf.getColumn(CellName.wrap("col1998")).value();
        assertEquals(ByteBufferUtil.string(col), "v1998");
        col = cf.getColumn(CellName.wrap("col1999")).value();
        assertEquals(ByteBufferUtil.string(col), "v1999");

        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("col9000"), CellName.EMPTY_CELL_NAME, true, 3);
        assertColumns(cf, "col1997", "col1998", "col1999");

        cf = cfStore.getColumnFamily(key, new QueryPath("Standard1"), CellName.wrap("col9000"), CellName.EMPTY_CELL_NAME, false, 3);
        assertColumns(cf);
    }
    ...
    public void testGetSliceFromSuperBasic() throws Throwable
    {
        ...
        SuperColumn sc = new SuperColumn(CellName.wrap("sc1"), Int32Type.instance);
        sc.addColumn(new Column(CellName.wrap(1), ByteBufferUtil.bytes("val1"), 1L));
        cf.addColumn(sc);
        rm.add(cf);
        rm.apply();
        ...
        Runnable verify = new WrappedRunnable()
        {
            public void runMayThrow() throws Exception
            {
                ColumnFamily cf = cfStore.getColumnFamily(ROW, new QueryPath("Super1"), CellName.EMPTY_CELL_NAME, CellName.EMPTY_CELL_NAME, false, 10);
                assertColumns(cf, "sc1");

                ByteBuffer val = cf.getColumn(CellName.wrap("sc1")).getSubColumn(CellName.wrap(1)).value();
    ...
    {
        IColumnContainer sc = cf == null ? null : ((IColumnContainer)cf.getColumn(CellName.wrap(scName)));
    ...
    public static void assertColumns(IColumnContainer container, String... columnNames)
    {
        ...
        for (IColumn column : columns)
        {
            try
            {
                L.add(ByteBufferUtil.string(column.name().bb));
            }
    ...
    {
        assertColumn(cf.getColumn(CellName.wrap(name)), value, timestamp);
    }
    ...
    public static void assertSubColumn(ColumnFamily cf, String scName, String name, String value, long timestamp)
    {
        ...
        assertColumn(sc.getSubColumn(CellName.wrap(name)), value, timestamp);
    }
