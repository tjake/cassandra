...
public class ArrayBackedSortedColumnsTest extends SchemaLoader
{
    ...
    {
        CellNameType type = CellNames.simpleDenseType(Int32Type.instance);
        ColumnFamily map = ArrayBackedSortedColumns.factory.create(metadata(), reversed);
        int[] values = new int[]{ 1, 2, 2, 3 };

        for (int i = 0; i < values.length; ++i)
            map.addColumn(new Column(type.make(values[reversed ? values.length - 1 - i : i])), HeapAllocator.instance);
    ...
    private void testAddAllInternal(boolean reversed)
    {
        ...
        for (int i = 0; i < values1.length; ++i)
            map.addColumn(new Column(type.make(values1[reversed ? values1.length - 1 - i : i])), HeapAllocator.instance);

        for (int i = 0; i < values2.length; ++i)
            map2.addColumn(new Column(type.make(values2[reversed ? values2.length - 1 - i : i])), HeapAllocator.instance);
    ...
    private void testGetCollectionInternal(boolean reversed)
    {
        ...
        for (int v : values)
            sorted.add(new Column(type.make(v)));
