...
public class SSTableRewriterTest extends SchemaLoader
{
    ...
    public void basicTest() throws InterruptedException
    {
        ...
        try (AbstractCompactionStrategy.ScannerList scanners = cfs.getCompactionStrategyManager().getScanners(sstables, nowInSec);
             ...
             CompactionIterator ci = new CompactionIterator(OperationType.COMPACTION, scanners.scanners, controller, UUIDGen.getTimeUUID()))
        {
    ...
    public void testNumberOfFilesAndSizes() throws Exception
    {
        ...
        int files = 1;
        try (ISSTableScanner scanner = s.getScanner(FBUtilities.nowInSeconds());
             CompactionController controller = new CompactionController(cfs, compacting, 0);
             LifecycleTransaction txn = cfs.getTracker().tryModify(compacting, OperationType.UNKNOWN);
             SSTableRewriter rewriter = new SSTableRewriter(cfs, txn, 1000, false, 10000000);
             CompactionIterator ci = new CompactionIterator(OperationType.COMPACTION, Collections.singletonList(scanner), controller, UUIDGen.getTimeUUID()))
        {
    ...
    public void testNumberOfFiles_abort() throws Exception
    {
        testNumberOfFiles_abort(new RewriterTest()
        {
            ...
            {
                try (CompactionIterator ci = new CompactionIterator(OperationType.COMPACTION, Collections.singletonList(scanner), controller, UUIDGen.getTimeUUID()))
                {
                    int files = 1;
                    while (ci.hasNext())
                    {
    ...
    private void testAbortHelper(boolean earlyException, boolean offline) throws Exception
    {
        ...
        if (!offline)
            cfs.addSSTable(s);
        Set<SSTableReader> compacting = Sets.newHashSet(s);
        try (ISSTableScanner scanner = compacting.iterator().next().getScanner(FBUtilities.nowInSeconds());
             CompactionController controller = new CompactionController(cfs, compacting, 0);
    ...
    public void testCanonicalView() throws Exception
    {
        ...
        assertEquals(1, sstables.size());
        boolean checked = false;
        try (ISSTableScanner scanner = sstables.iterator().next().getScanner(FBUtilities.nowInSeconds());
